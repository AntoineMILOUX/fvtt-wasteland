...
export class WastelandItemSheet extends ItemSheet {
  ...
  async getData() {
    ...
    let formData = {
      ...
      description: await TextEditor.enrichHTML(this.object.system.description, {
        async: true
      }),
    ...
    if (objectData.type == "don") {
      formData.sacrifice = await TextEditor.enrichHTML(this.object.system.sacrifice, {
        async: true
      })
    }
  ...
  activateListeners(html) {
    ...
    html.find('#add-predilection').click(ev => {
      ...
      let pred = []
      if (this.object.system.predilections) {
         pred = duplicate(this.object.system.predilections)
      } else {
         pred = []
      }
      pred.push({
        name: "Nouvelle prédilection",
        used: false
      })
      this.object.update({
        'data.predilections': pred
